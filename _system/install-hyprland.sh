...
declare -A _repoSources=(
    ...
    ["hyprland-contrib"]="github hyprwm/hyprland-contrib"
...
_usageFn () {
    cat << _EOH1
usage: $(basename "$0") [-help] [+nodeps] [+nosddm] [+latest] [+custom-addons ADDON1,ADDON2,..]
                        [+plugins PLUG1,PLUG2,..]
    ...
    -select-addons      Install custom addons, ',' separated. (need to specify with +noaddons)
                        Available:
                          (hyprpaper,hyprlock,hypridle,xdg-desktop-portal-hyprland,hyprland-plugins,hyprland-contrib)
    -plugins            Specify plugins to install in hyprland-plugins, ',' separated.
...
_parseArgumentsFn () {
    ...
    if [ -z "${_noAddons}" ]; then
        ...
        case ":$(sed 's, ,:,g' <<<"${_selectedAddons[*]}"):" in
            *":hyprland-plugins:"*) ;;
            *)
...
_dbiHyprlandFn () {
    _hyprlandVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "github" "Hyprland")" "${_hyprlandVersion}")"
    _downloadSourceFn \
...
_dbiWaylandProtocolsFn () {
    ...
    _waylandProtocolsVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_waylandProtocolsVersion}")"
    _downloadSourceFn \
...
_dbiWaylandFn () {
    ...
    _waylandVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_waylandVersion}")"
    _downloadSourceFn \
...
_dbiLibdisplayInfoFn () {
    ...
    _libdisplayInfoVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_libdisplayInfoVersion}")"
    _downloadSourceFn \
...
_dbiLibinputFn () {
    ...
    _libinputVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_libinputVersion}")"
    _downloadSourceFn \
...
_dbiLibliftoffFn () {
    ...
    _libliftoffVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_libliftoffVersion}")"
    _downloadSourceFn \
...
_dbiHyprcursorFn () {
    ...
    _hyprcursor="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_hyprcursorVersion}")"
    _downloadSourceFn \
...
_dbiHyprlangFn () {
    ...
    _hyprlangVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_hyprlangVersion}")"
    _downloadSourceFn \
...
_dbiHyprwaylandScannerFn () {
    ...
    _hyprwaylandScannerVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_hyprwaylandScannerVersion}")"
    _downloadSourceFn \
...
_dbiSddmFn () {
    ...
    _sddmVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_sddmVersion}")"
    _downloadSourceFn \
        "sddm-v${_sddmVersion}" \
        "$(_getSourceTarballLinkFn "${_repo_src[@]}" "${_sddmVersion}")"

    mkdir "sddm-${_sddmVersion}/build/" && \
    pushd "sddm-${_sddmVersion}/build/" || exit 10
...
_dbiHyprpaperFn () {
    ...
    _hyprpaperVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_hyprpaperVersion}")"
    _downloadSourceFn \
...
_dbiHyprlockFn () {
    ...
    _hyprlockVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_hyprlockVersion}")"
    _downloadSourceFn \
...
_dbiHypridleFn () {
    ...
    _hypridleVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_hypridleVersion}")"
    _downloadSourceFn \
...
_dbiXdgDesktopPortalHyprlandFn () {
    ...
    _xdgDesktopPortalHyprlandVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_xdgDesktopPortalHyprlandVersion}")"
    _downloadSourceFn \
    ...
    cmake -DCMAKE_INSTALL_LIBEXECDIR:PATH=/usr/lib -DCMAKE_INSTALL_PREFIX:PATH=/usr -B build \
        && cmake --build ./build -j"$(nproc 2>/dev/null || getconf _NPROCESSORS_CONF)" && \
    sudo cmake --install ./build
...
_dbiHyprlandPluginsFn () {
    ...
    _hyprlandPluginsVersion="$(_getLatestOrValidateVersionFn "$(_getSourceLinkFn "${_repo_src[@]}")" "${_hyprlandPluginsVersion}")"
    _downloadSourceFn \
    ...
    # TODO: finish the logic in hyprland-plugins
    _errMsg "No logic in ${FUNCNAME[0]}"
    popd || exit 1
    return
}
...
_dbiHyprlandContribFn () {
    ...
    # TODO: finish the logic in hyprland-contrib
    _errMsg "No logic in ${FUNCNAME[0]}"
    popd || exit 1
    return
}
...
_installAddonsFn () {
    # TODO: remove duplicates from _selectedAddons
    [ "${#_selectedAddons[@]}" -eq 0 ] && \
        _selectedAddons=(
            "hyprpaper"
            "hyprlock"
            "hypridle"
    ...
    for _addon in "${_selectedAddons[@]}"; do
        case "${_addon}" in
            "hyprpaper")
                _dbiHyprpaperFn ;;
            "hyprlock")
